...
def pollinator_abundance_calculation(plantation_id=9827, roi_id=284086, ca_id=284085):
    ...
    Args:
        plantation_id: ID of the plantation
        roi_id: ID of the Region of Interest (ROI)
        ca_id: ID of the Comparison Area (CA)
    ...
    plantations_polygons_id = 9773  # This could be made a parameter in the future if needed
    resolution = "low"
    override_bee = True
    how = "local"
    compute_pa_ns = True
    ...
    try:
        ...
        if compute_pa_ns is True:
            ...
            # Use more threads based on available CPU cores
            import os
            max_threads = min(os.cpu_count() or 4, len(bee_data))
            total_bee = 0
            ...
            # Process bees in batches to reduce memory pressure and thread contention
            batch_size = max(1, len(bee_data) // max_threads)

            with ThreadPoolExecutor(max_workers=max_threads) as executor:
                # Process in batches
                for i in range(0, len(bee_data), batch_size):
                    batch = bee_data[i:i+batch_size]
                    futures = [
                        executor.submit(
                            lambda_bee,
                            plantation_id,
